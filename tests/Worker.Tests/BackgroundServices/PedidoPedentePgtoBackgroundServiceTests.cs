...
public class PedidoPedentePgtoBackgroundServiceTests
{
    private readonly Mock<ISqsService<PedidoPendentePagamentoEvent>> _sqsServiceMock;
    private readonly Mock<IServiceScopeFactory> _serviceScopeFactoryMock;
    private readonly Mock<ILogger<PedidoPedentePgtoBackgroundService>> _loggerMock;
    private readonly Mock<IServiceScope> _serviceScopeMock;
    private readonly Mock<IServiceProvider> _serviceProviderMock;
    private readonly Mock<IPedidoRepository> _pedidoRepositoryMock;

    public PedidoPedentePgtoBackgroundServiceTests()
    {
        _sqsServiceMock = new Mock<ISqsService<PedidoPendentePagamentoEvent>>();
        _serviceScopeFactoryMock = new Mock<IServiceScopeFactory>();
        _loggerMock = new Mock<ILogger<PedidoPedentePgtoBackgroundService>>();
    ...
    public async Task ExecuteAsync_Should_ProcessMessages()
    {
        ...
        _sqsServiceMock.Setup(x => x.ReceiveMessagesAsync(It.IsAny<CancellationToken>())).ReturnsAsync(pedidoPendentePagamentoEvent);
        _pedidoRepositoryMock.Setup(x => x.FindByIdAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>())).ReturnsAsync(new PedidoDb { Id = pedidoPendentePagamentoEvent.PedidoId, Status = "Recebido" });
        ...
        var service = new PedidoPedentePgtoBackgroundService(_sqsServiceMock.Object, _serviceScopeFactoryMock.Object, _loggerMock.Object);
    ...
    public async Task ExecuteAsync_Should_UpdateStatus_When_ValidTransition()
    {
        ...
        var pedidoExistente = new PedidoDb { Id = pedidoPendentePagamentoEvent.PedidoId, Status = "Recebido" };
        _sqsServiceMock.Setup(x => x.ReceiveMessagesAsync(It.IsAny<CancellationToken>())).ReturnsAsync(pedidoPendentePagamentoEvent);
        ...
        Assert.Equal("PendentePagamento", pedidoExistente.Status);
    }
}
